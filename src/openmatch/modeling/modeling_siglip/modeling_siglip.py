...
class SiglipVisionEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.FloatTensor, patch_attention_mask: torch.BoolTensor) -> torch.Tensor:
        ...
        patch_embeds = self.patch_embedding(pixel_values.to(dtype))
...
class SiglipVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        # print(f"encoder_outputs[0] dtype = {encoder_outputs[0].dtype}")
...
class SiglipModel(SiglipPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs
        ...
        inputs = self.processor(text=text, images=image, padding="max_length", truncation=True, return_tensors="pt")
        pixel_values = None
        input_ids = None
        attention_mask = None
        ...
        if (pixel_values is not None):
            vision_outputs, attention_mask = self.vision_model(
                pixel_values=pixel_values,
                output_attentions=output_attentions,
                output_hidden_states=output_hidden_states,
                return_dict=return_dict,
        ...
        return SiglipOutput(
            last_hidden_state=last_hidden_state, 
            attention_mask=attention_mask,
            pooled_output = text_outputs[1] if text_outputs is not None else vision_outputs[1]
        )
