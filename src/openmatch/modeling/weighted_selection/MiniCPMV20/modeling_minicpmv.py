...
class MiniCPMV(MiniCPMVPreTrainedModel):
    ...
    def _decode(self, inputs_embeds, tokenizer, **kwargs):
        ...
        return self._decode_text(output.sequences, tokenizer), output.sequences_scores
    ...
    def generate(
        ...
        **kwargs
        ...
        with torch.inference_mode():
            ...
            result, sequences_scores = self._decode(model_inputs["inputs_embeds"], tokenizer, **kwargs)

        if return_vision_hidden_states:
            return result, vision_hidden_states, sequences_scores

        return result, sequences_scores
    ...
    def chat(
        ...
        **kwargs
        ...
        with torch.inference_mode():
            res, vision_hidden_states, sequences_scores = self.generate(
    ...
    def weighted_selection(self,
        ...
        **kwargs
        ...
        doc_scores = torch.nn.functional.softmax(doc_scores, dim=0)
        doc_scores = doc_scores.tolist()
        
        for i, image in enumerate(image_list):
            responds, sequences_scores = self.chat(
            image=image,
            msgs=msgs,
            context=None,
            tokenizer=tokenizer,
            ...
            responds_list.append(responds)
            scores_list.append(doc_scores[i] * math.exp(sequences_scores[0]))
        max_index = scores_list.index(max(scores_list))
        return responds_list[max_index]
