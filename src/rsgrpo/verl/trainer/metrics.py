...
def compute_data_metrics(batch: DataProto, use_critic: bool = False) -> Dict[str, Any]:
    ...
    advantages = batch.batch["advantages"].sum(dim=-1, keepdim=True).repeat((1,max_response_length))
    returns = batch.batch["returns"].sum(dim=-1, keepdim=True).repeat((1,max_response_length))
