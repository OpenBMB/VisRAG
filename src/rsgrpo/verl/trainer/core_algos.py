...
@torch.no_grad()
def compute_router_outcome_advantage(
    token_level_rewards: torch.Tensor, response_mask: torch.Tensor, index: torch.Tensor, eps: float = 1e-6
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Compute advantage for Reward Router, operating only on Outcome reward
    (with only one scalar reward for each response).

    Args:
        token_level_rewards: `(torch.Tensor)`
            shape: (bs, response_length) -> (bs, n_rewards)
        response_mask: `(torch.Tensor)`
            shape: (bs, response_length)
        index: `(torch.Tensor)`
            shape: (bs,)
    ...
    """
    scores = token_level_rewards # bs, n_rewards
    id2score = {}
    id2mean, id2std = {}, {}

    bsz, n_rewards = scores.shape[0], scores.shape[1]
    for i in range(bsz):
        for j in range(n_rewards):
            if index[i] not in id2score:
                id2score[index[i]] = [[] for _ in range(n_rewards)]
            id2score[index[i]][j].append(scores[i][j])

    for idx in id2score:
        for j in range(n_rewards):
            assert len(id2score[idx][j]) > 1, "GRPO needs rollout.n > 1."
            id2mean[f"{idx}_{j}"] = torch.mean(torch.tensor(id2score[idx][j]))
            id2std[f"{idx}_{j}"] = torch.std(torch.tensor(id2score[idx][j]))

    for i in range(bsz):
        for j in range(n_rewards):
            scores[i][j] = (scores[i][j] - id2mean[f"{index[i]}_{j}"]) / (id2std[f"{index[i]}_{j}"] + eps)
...
def average_loss(
    ...
    Args:
        values: `(torch.Tensor)`
            shape: (bs, n_rewards, response_length)
        mask: `(torch.Tensor)`
            shape: (bs, response_length) -> (bs, n_rewards, response_length)
    ...
    """
    if mode == "router":
        return (values * mask).sum(dim=(0,2)) / (mask.sum(dim=(0,2)) + eps)
    elif mode == "token":
...
def compute_policy_loss(
    ...
    advantages = advantages.unsqueeze(-1) * reward_masks

    negative_approx_kl = log_probs - old_log_probs
    # clamp negative_approx_kl to avoid nan kld
    negative_approx_kl = torch.clamp(negative_approx_kl, -20.0, 20.0).unsqueeze(-2).repeat(1,advantages.size(1),1)
    ...
    # pg metrics
    metrics = {"ppo_kl": -negative_approx_kl.unsqueeze(1)}
    ...
    final_pg_loss = average_loss(final_pg_loss, reward_masks, mode="router")

    metrics = {k: VF.masked_mean(v, reward_masks).detach().item() for k, v in metrics.items()}
