...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto) -> Dict[str, Any]:
        ...
        select_keys = ["input_ids", "attention_mask", "position_ids", "responses", "response_mask", "reward_masks"]
        ...
        for _ in range(self.config.ppo_epochs):
            ...
            for mini_batch in mini_batches:
                total_response_tokens = torch.sum(mini_batch.batch["reward_masks"], dim=(0,2))
                ...
                for micro_batch in micro_batches:
                    ...
                    reward_masks = model_inputs["reward_masks"]
                    old_log_probs = model_inputs["old_log_probs"]
                    advantages = model_inputs["advantages"] # B, n_rewards
                    ...
                    if self.config.use_kl_loss and "ref_log_probs" in model_inputs:
                        ...
                        kl_loss = average_loss(kld, reward_masks, mode=self.config.loss_avg_mode)
                    ...
                    loss = (loss * torch.sum(reward_masks, dim=(0,2)) * self.world_size / total_response_tokens).sum() / (loss != 0.0).sum()
                    # loss = (loss * torch.sum(reward_masks, dim=(0,2)) * self.world_size / total_response_tokens).sum()
                    loss.backward()
